  ...
  const contentSections = (
    <>
      <Section title="What We Do" subtitle="Practical, field-ready service scope tailored for industrial and mining-adjacent use.">
        <ul className="space-y-3 rounded-xl border border-slate-700 bg-brand-600 p-6 text-sm text-slate-100">
      ...
      <Section title="Typical Deliverables" subtitle="What clients can expect from each service engagement.">
        ...
        {service.extraSection ? (
          <div className="mt-6 rounded-xl border border-slate-700 bg-brand-700 p-6 text-slate-100">
            <h3 className="text-2xl font-bold">{service.extraSection.title}</h3>
    ...
    <>
      <section className="border-b border-slate-800 bg-gradient-to-b from-black to-brand-700 py-14 sm:py-16">
        <Container className="grid gap-8 lg:grid-cols-2 lg:items-center">
          <div>
            <h1 className="text-3xl font-bold text-white sm:text-5xl">{service.title}</h1>
            <p className="mt-4 text-base text-slate-200">{service.intro}</p>
            <div className="mt-6">
              <Button href={contactHref}>{service.ctaLabel || "Request Service Support"}</Button>
            </div>
          </div>
          {service.slug === "/core-business-2" ? (
            <div className="flex h-[280px] items-center justify-center overflow-hidden rounded-xl border border-slate-700 bg-black/30 p-3 shadow-sm sm:h-[340px]">
              <Image
